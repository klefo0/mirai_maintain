...
if(args[0] == "me"){
	 if (args[1] == "admin") {
		...
		if(!find) api.sendMessage("BOT cần đặt làm quản trị viên để dùng ?", event.threadID, event.messageID)
...
if (!finddd) return api.sendMessage("Stop there you criminal scum!!!\nCall the authority and get the admin or not using", event.threadID, event.messageID);		
if(!find) {api.sendMessage("The authority not giving admin role?\nGood luck next time =))", event.threadID, event.messageID)}
